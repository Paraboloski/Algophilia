...
class BaseRepository(Generic[T]):
    ...
    async def create(cls, entity: T) -> Result[T, IOError_]:
        ...
        return await attempt_async(
            _run(),
            lambda e: IOError_(message=f"Failed to create {cls._entity_name()}", target=str(e))
        )
    ...
    async def create_all(cls, entities: list[T]) -> Result[list[T], IOError_]:
        ...
        return await attempt_async(
            _run(),
            lambda e: IOError_(message=f"Failed to bulk-create {cls._entity_name()}", target=str(e))
        )
    ...
    async def get_by_id(cls, entity_id: int) -> Result[T, NotFoundError | IOError_]:
        ...
        res = await attempt_async(
            _run(),
            lambda e: IOError_(message=f"Failed to fetch {cls._entity_name()}", target=str(e))
        )
    ...
    async def get_all(cls) -> Result[Sequence[T], IOError_]:
        ...
        return await attempt_async(
            _run(),
            lambda e: IOError_(message=f"Failed to list {cls._entity_name()}", target=str(e))
        )
    ...
    async def update(cls, entity_id: int, data: dict) -> Result[T, NotFoundError | IOError_]:
        ...
        res = await attempt_async(
            _run(),
            lambda e: IOError_(message=f"Failed to update {cls._entity_name()}", target=str(e))
        )
    ...
    async def delete(cls, entity_id: int) -> Result[None, NotFoundError | IOError_]:
        ...
        res = await attempt_async(
            _run(),
            lambda e: IOError_(message=f"Failed to delete {cls._entity_name()}", target=str(e))
        )
