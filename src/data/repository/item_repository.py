...
from src.data.models.items import Item, ItemArtillery, ItemArmor, ItemMisc, Inventory, InventoryItem
...
class InventoryRepository(BaseRepository[Inventory]):
    ...
    async def get_by_character(cls, character_id: int) -> Result[Inventory, NotFoundError | IOError_]:
        ...
        res = await attempt_async(
            _run(),
            lambda e: IOError_(message="Failed to fetch Inventory by character", target=str(e))
        )
...
class ItemRepository(BaseRepository[Item]):
    ...
    async def get_with_details(cls, entity_id: int) -> Result[Item, NotFoundError | IOError_]:
        ...
        res = await attempt_async(
            _run(),
            lambda e: IOError_(message="Failed to fetch Item with details", target=str(e))
        )
    ...
    async def get_by_category(cls, category) -> Result[Sequence[Item], IOError_]:
        ...
        return await attempt_async(
            _run(),
            lambda e: IOError_(message="Failed to fetch Items by category", target=str(e))
        )
...
class InventoryItemRepository(BaseRepository[InventoryItem]):
    ...
    async def get_by_inventory(cls, inventory_id: int) -> Result[Sequence[InventoryItem], IOError_]:
        ...
        return await attempt_async(
            _run(),
            lambda e: IOError_(message="Failed to list InventoryItems", target=str(e))
        )
